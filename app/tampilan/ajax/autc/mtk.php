...
<ul class="autc">
	<?php if (!is_null($data)): ?>
		<?php foreach ($data as $hasil): ?>
			<li class="autc <?php if ($hasil['mtk_buka'] != 0 AND $hasil['mtk_tutup'] == 0) echo 'autc-mtk'; else echo 'text-danger'; ?>"><?php echo $hasil['mtk_id'] . ' - ' . $hasil['mtk_nama'] . ' - ' . $hasil['dsn_nama'] . ' (' . $hasil['mtk_periode'] . ')'; ?></li>
		<?php endforeach ?>
	<?php else: ?>
